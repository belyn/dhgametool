-- Command line was: E:\github\dhgametool\scripts\common\play800sdkcfg.lua 
...
cfg.pay = function(l_4_0, l_4_1)
  local oparams = {productId = l_4_0.productId, type = 34}
  addWaitNet()
  getOrder(oparams, function(l_1_0)
    delWaitNet()
    tablePrint(l_1_0)
    local netOrder = jsonDecode(l_1_0.order_info)
    local ucParams = {product_id = netOrder.product_id, product_name = netOrder.product_name, product_desc = netOrder.product_desc, server_name = netOrder.server_name, server_id = "" .. player.sid, role_id = "" .. player.uid, role_name = player.name, role_level = player.lv(), order_id = netOrder.cp_order_id, extradata = netOrder.extradata, notify_url = netOrder.notifyurl, channel = "1", price = netOrder.order_money}
...
cfg.submitRoleData = function(l_6_0, l_6_1)
  ...
  local sparams = {stype = l_6_0.stype or "1", role_id = l_6_0.roleId .. "", role_name = l_6_0.roleName .. "", role_level = checkint(l_6_0.roleLevel), server_id = l_6_0.zoneId .. "", server_name = l_6_0.zoneName .. "", balance = require("data.bag").gem() .. "", vip_level = player.vipLv() .. "", party_name = player.gname or "None"}
  cfg.need_submit = nil
