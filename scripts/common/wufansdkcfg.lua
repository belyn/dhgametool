-- Command line was: E:\github\dhgametool\scripts\common\wufansdkcfg.lua 
...
cfg.login = function(l_3_0, l_3_1)
  ...
  SDKHelper:getInstance():login("", function(l_1_0)
    ...
    if __data and __data.errcode and __data.errcode ~= 0 then
      ...
      local jparams = {openuid = __data.openuid, token = __data.token}
...
cfg.pay = function(l_6_0, l_6_1)
  local oparams = {productId = l_6_0.productId, type = 27}
  addWaitNet()
  getOrder(oparams, function(l_1_0)
    tablePrint(l_1_0)
    local netOrder = jsonDecode(l_1_0.order_info)
    local wfParams = {app_name = "\230\148\190\231\189\174\229\165\135\229\133\181", app_order_id = netOrder.app_order_id, product_name = netOrder.product_name, product_id = netOrder.product_id .. "", pa_open_id = netOrder.pa_open_id, notifyUrl = netOrder.notifyUrl, money_amount = netOrder.money_amount, role_id = (player.uid or "") .. "", role_name = player.name or "", server_id = player.sid .. ""}
