-- Command line was: E:\github\dhgametool\scripts\fight\gwarrep\video.lua 
...
ui.create = function(l_1_0)
  ...
  layer.getVideoAndUnits = function()
    ...
    for i,h in pairs(video.atk.camp) do
      attackers[i] = hHelper.createHero({id = h.id, lv = h.lv, pos = h.pos, star = h.star, side = "attacker", ep = h.energy, hp = h.hp, wake = h.wake, skin = h.skin})
    end
    local defenders = {}
    for i,h in pairs(video.def.camp) do
      defenders[i] = hHelper.createHero({id = h.id, lv = h.lv, pos = 6 + h.pos, star = h.star, side = "defender", ep = h.energy, hp = h.hp, wake = h.wake, skin = h.skin})
    end
