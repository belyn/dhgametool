-- Command line was: E:\github\dhgametool\scripts\ui\loginreward\main.lua 
...
ui.createContent = function()
  local layer = CCLayer:create()
  local board = CCSprite:create()
  board:setContentSize(CCSize(512, 392))
  board:setAnchorPoint(ccp(0, 0))
  board:setPosition(scalep(302, 63))
  board:setScale(view.minScale)
  layer:addChild(board)
  local board_w = board:getContentSize().width
  local board_h = board:getContentSize().height
  local chat = img.createLogin9Sprite(img.login.toast_bg)
  chat:setPreferredSize(CCSize(570, 68))
  chat:setAnchorPoint(CCPoint(0.5, 0))
  chat:setPosition(board_w / 2, 8)
  board:addChild(chat, 10)
  local scroll = CCScrollView:create()
  scroll:setDirection(kCCScrollViewDirectionVertical)
  scroll:setPosition(4, 74)
  scroll:setViewSize(CCSize(512, 322))
  scroll:setContentSize(CCSize(512, 616))
  board:addChild(scroll)
  scroll:setContentOffset(ccp(0, -294))
  if monthlogin.recvdays >= 15 then
    scroll:setContentOffset(ccp(0, 0))
  end
  local shadow = img.createUI9Sprite(img.ui.login_reward_shadow2)
  shadow:setAnchorPoint(0, 0)
  shadow:setPreferredSize(CCSize(520, 12))
  shadow:setPosition(0, 74)
  board:addChild(shadow)
  local getposition = function(l_1_0)
    local x0 = 52
    local y0 = scroll:getContentSize().height - 53
    local x = x0 + math.floor((l_1_0 - 1) % 5) * 102
    local y = y0
    y = y - math.floor((l_1_0 - 1) / 5) * 102
    return x, y
   end
  local todayNum = nil
  local borderBg = {}
  ...
  for i = 1, 30 do
    ...
    borderBg[i] = img.createUISprite(img.ui.login_reward_recet)
    ...
    if monthlogin.daily[i] == 3 then
      local _mask = img.createUISprite(img.ui.login_reward_shadow)
      _mask:setPosition(CCPoint(x, y))
      scroll:getContainer():addChild(_mask, 100)
      local tickIcon = img.createUISprite(img.ui.login_month_finish)
      tickIcon:setPosition(x + 28, y + 28)
      scroll:getContainer():addChild(tickIcon, 100)
  ...
  checkinBtn:registerScriptTapHandler(function()
    ...
    net:alogin(param, function(l_1_0)
      ...
      if cfgmonthlogin[i + offset_idx].rewards[1].type == 1 then
        ...
        layer:getParent():addChild(pop, 100)
      else
      ...
      local _mask = img.createUISprite(img.ui.login_reward_shadow)
      _mask:setPosition(CCPoint(nowx, nowy))
      scroll:getContainer():addChild(_mask, 100)
      ...
      local tickIcon = img.createUISprite(img.ui.login_month_finish)
      tickIcon:setPosition(nowx + 28, nowy + 28)
      scroll:getContainer():addChild(tickIcon, 100)
      end)
   end)
  if isCheckin() == 0 then
          ...
          do
            ...
            if layer.tipsTag == false then
              ...
              if cfgmonthlogin[i + offset_idx].rewards[1].type == 1 then
                ...
                layer:getParent():addChild(tips, 200)
                tips.setClickBlankHandler(function()
                  tips:removeFromParent()
                  layer.tipsTag = false
                        end)
  ...
  local addBan = function(l_10_0, l_10_1, l_10_2, l_10_3)
    local ban = CCLayer:create()
    l_10_0:addChild(ban, 1000, 101)
    ...
    ban:registerScriptTouchHandler(function(l_1_0, l_1_1, l_1_2)
      if l_1_0 == "began" then
        ...
        local p2 = nil
        if board and board:isEnabled() then
          p2 = board:getParent():convertToNodeSpace(ccp(l_1_1, l_1_2))
        end
        local p3 = node:getParent():convertToNodeSpace(ccp(l_1_1, l_1_2))
        if not scroll:boundingBox():containsPoint(p1) and ((p2 and board:boundingBox():containsPoint(p2)) or node:boundingBox():containsPoint(p3)) then
  ...
  addBan(layer, scroll, checkinBtn, detailBtn)
  return layer
end

ui.create = function()
  img.load(img.packedOthers.ui_login_reward)
  local layer = CCLayer:create()
  local darkbg = CCLayerColor:create(ccc4(0, 0, 0, POPUP_DARK_OPACITY * 0.8))
  layer:addChild(darkbg)
  local bg = img.createUISprite(img.ui.login_reward_bg)
  bg:setPosition(view.midX - 44 * view.minScale, view.midY)
  bg:setAnchorPoint(0.5, 0.5)
  bg:setScale(view.minScale)
  layer:addChild(bg)
  local title = lbl.createFont2(24, i18n.global.activity_des_login.string, ccc3(250, 216, 105))
  title:setPosition(495, 492)
  bg:addChild(title)
  local btn_close0 = img.createUISprite(img.ui.close)
  local btn_close = SpineMenuItem:create(json.ui.button, btn_close0)
  btn_close:setPosition(scalep(830, 517))
  ...
  layer:addChild(ui.createContent())
  addBackEvent(layer)
  ...
  layer:registerScriptHandler(function(l_3_0)
    ...
    elseif l_3_0 == "cleanup" then
      img.unload(img.packedOthers.ui_login_reward)
    end
   end)
  layer:setTouchEnabled(true)
