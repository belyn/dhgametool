-- Command line was: E:\github\dhgametool\scripts\ui\tips\summon.lua 
...
tips.create = function(l_1_0, l_1_1, l_1_2)
  ...
  bg:addChild(addMenu)
  local costCount = math.floor(l_1_1.num / cfgitem[l_1_1.id].heroCost.count)
  ...
  local summonLabel = lbl.createFont1(18, i18n.global.tips_summon.string, ccc3(115, 59, 5))
  ...
  summonBtn:registerScriptTapHandler(function()
    ...
    if handler then
      handler({id = thing.id, num = inputBox.num * cfgitem[thing.id].heroCost.count})
    end
   end)
  ...
  if l_1_1.num < cfgitem[l_1_1.id].heroCost.count then
  ...
  progressFg:setPercentage(l_1_1.num / cfgitem[l_1_1.id].heroCost.count * 100)
  bg:addChild(progressFg)
  local str = string.format("%d/%d", cfgitem[l_1_1.id].heroCost.count * costCount, cfgitem[l_1_1.id].heroCost.count)
  ...
  subBtn:registerScriptTapHandler(function()
    ...
    if inputBox.num > 1 then
      ...
      upvalue_1024 = string.format("%d/%d", cfgitem[thing.id].heroCost.count * inputBox.num, cfgitem[thing.id].heroCost.count)
      label:setString(str)
    end
   end)
  addBtn:registerScriptTapHandler(function()
    ...
    if inputBox.num < costCount then
      inputBox.num = inputBox.num + 1
      inputBox:setText(tostring(inputBox.num))
      upvalue_1536 = string.format("%d/%d", cfgitem[thing.id].heroCost.count * inputBox.num, cfgitem[thing.id].heroCost.count)
      label:setString(str)
    end
   end)
