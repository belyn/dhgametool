-- Command line was: E:\github\dhgametool\scripts\ui\setting\help.lua 
...
ui.create = function()
  ...
  local showContent = function(l_6_0)
    ...
    scroll.addSpace(55)
    for ii = 1, #l_6_0.describe do
      if l_6_0.describe[ii].title then
        local item_title = lbl.createMix({font = 1, size = 18, text = l_6_0.describe[ii].title, color = ccc3(148, 98, 66), width = 644, align = kCCTextAlignmentLeft})
        item_title.height = item_title:getContentSize().height * item_title:getScaleY()
        item_title.ax = 0
        item_title.px = 10
        scroll.addItem(item_title)
      end
      if l_6_0.describe[ii].content then
        local item_content = lbl.createMix({font = 1, size = 18, text = l_6_0.describe[ii].content, color = ccc3(148, 98, 66), width = 644, align = kCCTextAlignmentLeft})
