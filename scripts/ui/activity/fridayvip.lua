-- Command line was: E:\github\dhgametool\scripts\ui\activity\fridayvip.lua 
...
local vp_ids = {IDS.FRIDAY_VIP_1.ID, IDS.FRIDAY_VIP_2.ID, IDS.FRIDAY_VIP_3.ID, IDS.FRIDAY_VIP_4.ID, IDS.FRIDAY_VIP_5.ID, IDS.FRIDAY_VIP_6.ID, IDS.FRIDAY_VIP_7.ID, IDS.FRIDAY_VIP_8.ID, IDS.FRIDAY_VIP_9.ID, IDS.FRIDAY_VIP_10.ID, IDS.FRIDAY_VIP_11.ID, IDS.FRIDAY_VIP_12.ID, IDS.FRIDAY_VIP_13.ID, IDS.FRIDAY_VIP_14.ID}
ui.create = function()
  ...
  img.load(img.packedOthers.ui_activity_friday_vip)
  local banner = img.createUISprite(img.ui.activity_friday_vip_banner)
  local suffix = "us"
  ...
  local bannerLabel = img.createUISprite(string.format("activity_friday_vip_%s.png", suffix))
  bannerLabel:setAnchorPoint(CCPoint(0.5, 1))
  bannerLabel:setPosition(CCPoint(board_w / 2 + 74, board_h - 25))
  ...
  local createItem = function(l_2_0)
    ...
    btn0:setPreferredSize(CCSizeMake(128, 45))
    local lock = player.vipLv() < cfgobj.instruct
    if lock then
      local lockIcon = img.createUISprite(img.ui.icon_lock)
      lockIcon:setPosition(CCPoint(27, btn0:getContentSize().height / 2))
      btn0:addChild(lockIcon)
      local lbl_btn = lbl.create({font = 1, size = 18, text = string.format("VIP%d", cfgobj.instruct), color = ccc3(115, 59, 5), pt = {size = 14}, es = {size = 14}})
      lbl_btn:setPosition(CCPoint(83, 23))
    ...
    btn:registerScriptTapHandler(function()
      ...
      layer:getParent():getParent():addChild(createSurebuy(commit), 1000)
      end)
    return temp_item
   end
  local lineScroll = require("ui.lineScroll")
  local scroll_params = {width = 550, height = 279}
  ...
  layer:registerScriptHandler(function(l_7_0)
    ...
    if l_7_0 == "exit" then
      img.unload(img.packedOthers.ui_activity_friday_vip)
    end
   end)
