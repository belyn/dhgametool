-- Command line was: E:\github\dhgametool\scripts\ui\activity\tinysnowman.lua 
...
local vp_ids = {IDS.CHRISTMAS_1.ID, IDS.CHRISTMAS_2.ID, IDS.CHRISTMAS_3.ID, IDS.CHRISTMAS_4.ID, IDS.CHRISTMAS_5.ID, IDS.CHRISTMAS_6.ID, IDS.CHRISTMAS_7.ID, IDS.CHRISTMAS_8.ID, IDS.CHRISTMAS_9.ID, IDS.CHRISTMAS_10.ID}
ui.create = function()
  ...
  local banner = (img.createUISprite("ui_christmas_board.png"))
  local banner_text = nil
  if i18n.getCurrentLanguage() == kLanguageKorean then
    banner_text = img.createUISprite("ui_christmas_board_kr.png")
  else
    if i18n.getCurrentLanguage() == kLanguageChineseTW then
      banner_text = img.createUISprite("ui_christmas_board_tw.png")
    else
      if i18n.getCurrentLanguage() == kLanguageJapanese then
        banner_text = img.createUISprite("ui_christmas_board_jp.png")
      else
        if i18n.getCurrentLanguage() == kLanguagePortuguese then
          banner_text = img.createUISprite("ui_christmas_board_pt.png")
        else
          if i18n.getCurrentLanguage() == kLanguageChinese then
            banner_text = img.createUISprite("ui_christmas_board_cn.png")
          else
            banner_text = img.createUISprite("ui_christmas_board_us.png")
          end
        end
      end
    end
  end
  banner_text:setPosition(CCPoint(368, 116))
  banner:addChild(banner_text)
  ...
  lbl_cd:setPosition(CCPoint(290, 20))
  ...
  lbl_cd_des:setPosition(CCPoint(365, 20))
  banner:addChild(lbl_cd_des)
  if i18n.getCurrentLanguage() == kLanguageRussian then
    lbl_cd_des:setPosition(CCPoint(250, 20))
    lbl_cd:setAnchorPoint(CCPoint(0, 0.5))
  ...
  coin_bg:setPosition(CCPoint(350, 50))
  banner:addChild(coin_bg)
  local coin_icon = img.createItemIcon2(ITEM_ID_ABS_CRYSTAL)
  ...
  local updateCoin = function()
    local itemObj = bagData.items.find(ITEM_ID_ABS_CRYSTAL)
    if not itemObj then
      itemObj = {id = ITEM_ID_ABS_CRYSTAL, num = 0}
    end
    lbl_coin:setString(itemObj.num)
   end
  updateCoin()
  local createSurebuy = function(l_2_0, l_2_1, l_2_2)
    local params = {}
    params.btn_count = 0
    ...
    btnYes:registerScriptTapHandler(function()
      ...
      if itemObj.num < cfgObj.extra[1].num then
        showToast(i18n.global.flowercake_notenough.string)
        return 
      end
  ...
  local buyCount = 1
  local selectSweepnumLayer = function(l_3_0, l_3_1, l_3_2)
    ...
    if l_3_1.rewards[1].type == ItemType.Equip then
      icon_thing = img.createEquip(l_3_1.rewards[1].id, l_3_1.rewards[1].num)
    else
      if l_3_1.rewards[1].type == ItemType.Item then
        icon_thing = img.createItem(l_3_1.rewards[1].id, l_3_1.rewards[1].num)
      end
    end
    ...
    local icon_broken = img.createItemIcon2(ITEM_ID_ABS_CRYSTAL)
    ...
    local brokennum = 0
    if bagData.items.find(ITEM_ID_ABS_CRYSTAL) then
      brokennum = bagData.items.find(ITEM_ID_ABS_CRYSTAL).num
    end
    local lbl_pay = lbl.createFont2(16, l_3_1.extra[1].num)
  ...
  local createItem = function(l_4_0)
    local cfgObj = cfgactivity[l_4_0.id]
    local temp_item = img.createUISprite("ui_christmas_item_board.png")
    ...
    local icon = img.createItemIcon2(ITEM_ID_ABS_CRYSTAL)
    icon:setScale(0.8)
    ...
    btn:registerScriptTapHandler(function()
      ...
      if vpObj.limits == 1 then
        ...
        layer:getParent():getParent():getParent():addChild(surelayer, 1000)
      else
        local selectsweepnumlayer = selectSweepnumLayer(vpObj, cfgObj, setAlreadyBuy)
        layer:getParent():getParent():getParent():addChild(selectsweepnumlayer, 1000)
      end
      end)
    return temp_item
   end
  local lineScroll = require("ui.lineScroll")
  local scroll_params = {width = 574, height = 236}
  local scroll = lineScroll.create(scroll_params)
  scroll:setAnchorPoint(CCPoint(0, 0))
  scroll:setPosition(CCPoint(2, 6))
  board:addChild(scroll)
  layer.scroll = scroll
  local sortValue = function(l_5_0)
    if l_5_0.limits <= 0 then
      return 10000 + l_5_0.id
    else
      return l_5_0.id
    end
   end
  ...
  local showList = function(l_6_0)
    for ii = 1,  l_6_0 do
      ...
      local tmp_item = createItem(l_6_0[ii])
      tmp_item.obj = l_6_0[ii]
      tmp_item:setPosition(CCPoint(_x + 8, _y - 55))
      scroll.content_layer:addChild(tmp_item)
    end
    local content_h = 60 - start_y - math.floor(( l_6_0 + ITEM_PER_ROW - 1) / ITEM_PER_ROW - 1) * step_y - step_y / 2
