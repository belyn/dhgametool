...
DecTable *NewTable(int r, Function * F, int b, int c) // Lua5.1 specific
{
   ...
   self->arraySize = luaO_fb2int(b); //by marvin
   self->keyedSize = luaO_fb2int(c); //by marvin
   
   PENDING(r) = 1;
   return self;
}

void AddToTable(Function* F, DecTable * tbl, char *value, char *key, char bDoPrint)
{
   DecTableItem *item;
   List *type;
   int index;
   ...
   if (bDoPrint && tbl->keyedSize == tbl->used && tbl->arraySize == 0) {
...
{
   int i;
   DecTable *tbl = LastItem(&(F->tables));
   if (tbl == NULL) {
   ...
   for (i = 1; i <= b; i++) {
      ...
      if (error)
         return;
      AddToTable(F, tbl, rstr, NULL, 0); // Lua5.1 specific TODO: it's not really this :(
      if (error)
         return;
   }
   //comment by marvin
